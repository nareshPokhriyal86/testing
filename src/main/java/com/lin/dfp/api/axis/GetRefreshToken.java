...
public class GetRefreshToken {
	...
	// This callback URL will allow you to copy the token from the success
	// screen.
	private static final String CALLBACK_URL = "http://localhost:8888/oauth2callback";//"urn:ietf:wg:oauth:2.0:oob";

	private static Credential getOAuth2Credential(
		...
		String authorizationCode = new BufferedReader(new InputStreamReader(
				System.in)).readLine();
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			clientSecrets = new GoogleClientSecretsBuilder().forApi(Api.DFP)
					.fromFile("env/dfp.properties").build();
		} catch (ValidationException e) {
			System.err
					.println("Please input your client ID and secret into your ads.properties file, which is either "
