...
/*	public String generateNewNexageReport(String startDate,String endDate,String dimensions){
    ...
    public List<String> getAllAccountIdByCompanyName(String companyName) {
    	...
    	try{
    		if(companyName!= null) {
            	...
            	if(accountsObjList!=null && accountsObjList.size()>0){
            		for (AccountsEntity accountsObj : accountsObjList) {
    					if(accountsObj !=null && accountsObj.getStatus() != null && accountsObj.getStatus().equals(LinMobileConstants.STATUS_ARRAY[0]) && accountsObj.getAccountDfpId()!=null && !accountsObj.getAccountDfpId().equals("")){
    						accountIdList.add(accountsObj.getAccountDfpId());
    					}
    				}
            	}
    ...
    public FinalisedTableDetailsObj loadFinaliseNonFinaliseObject(String id){
    	log.info("Going to load object with id:"+id);
    	IReportDAO reportDAO=new ReportDAO();
    	try {
			FinalisedTableDetailsObj obj=reportDAO.loadFinaliseOrNonFinaliseTable(id);
			return obj;
		} catch (DataServiceException e) {
			log.severe("Failed to load row from datastore:"+id+" : err msg:"+e.getMessage());	
			return null;
   ...
   public String uploadProcessFileAtBQ(FinalisedTableDetailsObj reportObject,int finalizeOrNonFinalize, 
		...
		if(finalizeOrNonFinalize==0){
   		...
   		if(reportsResponse!=null && reportsResponse.contains("Success")){	
   		   ...
   		   if(saveReportStatus && finalizeOrNonFinalize==1){    
   			   ...
   			   String nonFinaliseTableIdStr=bqDTO.getBigQuerySchemaName()+"_"+reportObject.getDataSource()+"_"+reportObject.getStartDate().replaceAll("-", "_");
   			   if(nonFinaliseTableId !=null ){
   				nonFinaliseTableIdStr=nonFinaliseTableId;
   			   }   			   
   			  
   			   try {
   				    BigQueryUtil.deleteTable(bqDTO.getServiceAccountEmail(), bqDTO.getServicePrivateKey(),
     					   bqDTO.getBigQueryProjectId(),  bqDTO.getBigQueryDataSetId(), nonFinaliseTableIdStr);
     			   
     			    IReportDAO reportDAO=new ReportDAO();
					reportDAO.deleteFinaliseOrNonFinaliseTable(nonFinaliseTableIdStr,publisherId);
					
					//Update non-finalise object as merged non-finalise object 
					String id=publisherId+":"+nonFinaliseTableIdStr;
					reportObject.setId(id);
					reportObject.setTableId(nonFinaliseTableIdStr);
					reportObject.setTableType(0);
					reportObject.setMergeStatus(1);					
					saveReportInDatastore(reportObject);
					
				  } catch (DataServiceException e) {
					log.severe("Failed to delete row from datastore:"+nonFinaliseTableIdStr+" : err msg:"+e.getMessage());					
				  }catch(Exception e){
						 log.severe("Exception :"+e.getMessage());
				 }       		   
   		   }
	...
	public List<String> generateDFPReportByProductPerformance(String rawFileName,String proccessFileName,
	 ...
	 @Override
	 public void saveProcessFileObj(ProcessFileObj processFileObj) throws DataServiceException{
		 IReportDAO reportDAO=new ReportDAO();
		 reportDAO.saveObjectStrongObfy(processFileObj);
	 }
	 
	 @Override
	 public List<ProcessFileObj> loadAllProcessObjList(String mapKey, int uploadState) throws Exception{
		 List<ProcessFileObj> processFileObjList = new ArrayList<>();
		 IReportDAO reportDAO=new ReportDAO();
		 processFileObjList = reportDAO.loadAllProcessObjList(mapKey, uploadState);
		return processFileObjList;
	 }
	 
	 @Override
	 public  void  saveDailyDataProcessObj(DailyDataProcessObj dailyDataProcessObj, String key) throws DataServiceException{
		 synchronized (this) {
			 IReportDAO reportDAO=new ReportDAO();
			 String memcacheKey = "DAILY_DATA_PROCESS_KEY";
			 Map<String,DailyDataProcessObj> dailyDataProcessObjMap = new HashMap<>();
			 dailyDataProcessObjMap =  (Map<String, DailyDataProcessObj>) MemcacheUtil.getObjectsMapFromCache(memcacheKey);
			 if(dailyDataProcessObjMap!=null){
			 dailyDataProcessObjMap.put(key, dailyDataProcessObj);
			 MemcacheUtil.setObjectsMapInCache(dailyDataProcessObjMap, memcacheKey);
			 log.info("key save in memcache");
			 }
			 reportDAO.saveObjectStrongObfy(dailyDataProcessObj);
		 }		 
	 }
	 ...
	 public   DailyDataProcessObj getDailyDataProcessObj(String processKey) throws Exception{
		...
		// dailyDataProcessObj = reportDAO.getDailyDataProcessObj(processKey);
		 String memcacheKey = "DAILY_DATA_PROCESS_KEY";
		 Map<String,DailyDataProcessObj> dailyDataProcessObjMap = new HashMap<>();
		 dailyDataProcessObjMap =  (Map<String, DailyDataProcessObj>) MemcacheUtil.getObjectsMapFromCache(memcacheKey);
		 if(dailyDataProcessObjMap!=null && dailyDataProcessObjMap.containsKey(processKey)){
			  dailyDataProcessObj = dailyDataProcessObjMap.get(processKey);
			  log.info("key found in memcache");
		 }else{
			  dailyDataProcessObj = reportDAO.getDailyDataProcessObj(processKey);
			  log.info("key not found in memcache");
			  if(dailyDataProcessObjMap!=null && !dailyDataProcessObjMap.containsKey(processKey)){
				  dailyDataProcessObjMap.put(processKey, dailyDataProcessObj);
				  MemcacheUtil.setObjectsMapInCache(dailyDataProcessObjMap, memcacheKey);
				  log.info("key save in memcache");
			  }
			  else if(dailyDataProcessObjMap==null){
				  dailyDataProcessObjMap = new HashMap<>();
				  dailyDataProcessObjMap.put(processKey, dailyDataProcessObj);
				  MemcacheUtil.setObjectsMapInCache(dailyDataProcessObjMap, memcacheKey);
				  log.info("key save in memcache");
			  }
