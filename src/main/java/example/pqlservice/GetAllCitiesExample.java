...
public static void runExample(DfpServices dfpServices, DfpSession session,String type, String filePathString) throws Exception {

	// Get the PublisherQueryLanguageService.
 PublisherQueryLanguageServiceInterface pqlService = dfpServices.get(session, PublisherQueryLanguageServiceInterface.class);
 ...
 do {
   ...
   System.out.printf("%d) %d criteria beginning at offset %d were found.\n", i++,
       resultSet.getRows() == null ? 0 : resultSet.getRows().length, statementBuilder.getOffset());

   statementBuilder.increaseOffsetBy(10000);
 } while (resultSet.getRows() != null && resultSet.getRows().length > 0);

 // Change to your file location.
 String filePath = new File("C:\\testdownload\\"+filePathString+".csv").toString();
...
DfpSession dfpSession = new DfpSession.Builder()
 ...
 //runExample(dfpServices, dfpSession,"COUNTRY","COUNTRY");
 runExample(dfpServices, dfpSession,"DMA_Region","DMA_Region");
// runExample(dfpServices, dfpSession,"STATE","STATE");
 //runExample(dfpServices, dfpSession,"CITY","CITY");
}
}