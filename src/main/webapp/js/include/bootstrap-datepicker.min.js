! function (e) {
    ...
    var r = function (t, n) {
        ...
        this.isRTL = i[this.language].rtl || false;
        this.format = s.parseFormat(n.format || this.element.data("date-format") || i[this.language].format || "mm/dd/yyyy");
        this.isInline = false;
        ...
        e(document).on("mousedown", function (t) {
            if (e(t.target).closest(".datepicker.datepicker-inline, .datepicker.datepicker-dropdown").length === 0) {
                r.hide()
            }
        });
        ...
        this.todayBtn = n.todayBtn || this.element.data("date-today-btn") || false;
        this.todayHighlight = n.todayHighlight || this.element.data("date-today-highlight") || false;
        this.calendarWeeks = false;
        if ("calendarWeeks" in n) {
            this.calendarWeeks = n.calendarWeeks
        } else if ("dateCalendarWeeks" in this.element.data()) {
            this.calendarWeeks = this.element.data("date-calendar-weeks")
        }
        if (this.calendarWeeks) this.picker.find("tfoot th.today").attr("colspan", function (e, t) {
                return parseInt(t) + 1
            });
    ...
    r.prototype = {
        ...
        getFormattedDate: function (e) {
            if (e === undefined) e = this.format;
            return s.formatDate(this.date, e, this.language)
        },
        setStartDate: function (e) {
            this.startDate = e || -Infinity;
            if (this.startDate !== -Infinity) {
                this.startDate = s.parseDate(this.startDate, this.format, this.language)
            }
            this.update();
        ...
        setEndDate: function (e) {
            ...
            if (this.endDate !== Infinity) {
                this.endDate = s.parseDate(this.endDate, this.format, this.language)
            }
            this.update();
            this.updateNavArrows()
        },
        ...
        place: function () {
            ...
            var n = this.component ? this.component.offset() : this.element.offset();
            var r = this.component ? this.component.outerHeight(true) : this.element.outerHeight(true);
            /* added for dynamic position of the datepopup */
            var leftPos = (n.left + this.height) - (this.element.outerHeight() + this.picker.outerHeight());
            
            this.picker.css({
                top: n.top + r,
                left: leftPos,/*chaged n.left to topPos*/
                zIndex: t
            })
        },
        update: function () {
            ...
            this.date = s.parseDate(e, this.format, this.language);
            if (t) this.setValue();
            var n = this.viewDate;
            ...
            } if (n && n.getTime() != this.viewDate.getTime()) {
                this.element.trigger({
                    type: "changeDate",
                    date: this.viewDate
                })
        ...
        fill: function () {
            var n = new Date(this.viewDate),
                ...
                h = new Date;
            this.picker.find(".datepicker-days thead th.switch").text(i[this.language].months[o] + " " + r);
            this.picker.find("tfoot th.today").text(i[this.language].today).toggle(this.todayBtn !== false);
            this.updateNavArrows();
            this.fillMonths();
            var p = t(r, o - 1, 28, 0, 0, 0, 0),
                d = s.getDaysInMonth(p.getUTCFullYear(), p.getUTCMonth());
            p.setUTCDate(d);
            p.setUTCDate(d - (p.getUTCDay() - this.weekStart + 7) % 7);
            var v = new Date(p);
            ...
            while (p.valueOf() < v) {
                if (p.getUTCDay() == this.weekStart) {
                    m.push("<tr>");
                    if (this.calendarWeeks) {
                        var y = new Date(p.getUTCFullYear(), p.getUTCMonth(), p.getUTCDate() - p.getDay() + 10 - (this.weekStart && this.weekStart % 7 < 5 && 7)),
                            b = new Date(y.getFullYear(), 0, 4),
                            w = ~~ ((y - b) / 864e5 / 7 + 1.5);
                        m.push('<td class="cw">' + w + "</td>")
                    }
                }
                g = "";
                ...
                if (this.todayHighlight && p.getUTCFullYear() == h.getFullYear() && p.getUTCMonth() == h.getMonth() && p.getUTCDate() == h.getDate()) {
                    g += " today"
                }
                if (c && p.valueOf() == c) {
                    g += " active"
                ...
                m.push('<td class="day' + g + '">' + p.getUTCDate() + "</td>");
                if (p.getUTCDay() == this.weekEnd) {
                    m.push("</tr>")
                }
                p.setUTCDate(p.getUTCDate() + 1)
            }
            this.picker.find(".datepicker-days tbody").empty().append(m.join(""));
            var E = this.date && this.date.getUTCFullYear();
            ...
            if (r == u) {
                S.slice(0, a).addClass("disabled")
            }
            if (r == f) {
                S.slice(l + 1).addClass("disabled")
            }
            m = "";
            ...
            r -= 1;
            for (var T = -1; T < 11; T++) {
                m += '<span class="year' + (T == -1 || T == 10 ? " old" : "") + (E == r ? " active" : "") + (r < u || r > f ? " disabled" : "") + '">' + r + "</span>";
                r += 1
            }
            x.html(m)
        },
        updateNavArrows: function () {
            ...
            case 0:
                if (this.startDate !== -Infinity && t <= this.startDate.getUTCFullYear() && n <= this.startDate.getUTCMonth()) {
                ...
                } if (this.endDate !== Infinity && t >= this.endDate.getUTCFullYear() && n >= this.endDate.getUTCMonth()) {
            ...
            case 1:
            case 2:
                if (this.startDate !== -Infinity && t <= this.startDate.getUTCFullYear()) {
                    this.picker.find(".prev").css({
                        visibility: "hidden"
                    })
                } else {
                ...
                } if (this.endDate !== Infinity && t >= this.endDate.getUTCFullYear()) {
                    this.picker.find(".next").css({
                        visibility: "hidden"
                    })
                } else {
        ...
        click: function (n) {
            ...
            if (r.length == 1) {
                ...
                case "th":
                    ...
                    case "today":
                        var o = new Date;
                        o = t(o.getFullYear(), o.getMonth(), o.getDate(), 0, 0, 0);
                        this.showMode(-2);
                ...
                case "span":
                    if (!r.is(".disabled")) {
                        ...
                        if (r.is(".month")) {
                            ...
                            this.viewDate.setUTCFullYear(f);
                            this.element.trigger({
                                type: "changeYear",
                                date: this.viewDate
                            })
                ...
                case "td":
                    if (r.is(".day") && !r.is(".disabled")) {
                        ...
                        var f = this.viewDate.getUTCFullYear(),
        ...
        showMode: function (e) {
            if (e) {
                this.viewMode = Math.max(0, Math.min(2, this.viewMode + e))
            }
    ...
    e.fn.datepicker = function (t) {
        ...
        return this.each(function () {
            var i = e(this),
                ...
                o = typeof t == "object" && t;
            if (!s) {
                i.data("datepicker", s = new r(this, e.extend({}, e.fn.datepicker.defaults, o)))
            }
