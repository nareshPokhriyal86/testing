  ...
  var render = function (responseXML, doc) {
    ...
    doc.styles = styles;
      ...
      for (pm = 0; pm < placemarkNodes.length; pm++) {
        ...
        //derive the ID using XPATH expression.
        // XPATH did not work so doing this by navigation
        var extData,data, id = null
       try
       {

                   extData = node.getElementsByTagName("ExtendedData")[0];
                   data = extData.getElementsByTagName("Data")[0];
                   id = geoXML3.nodeValue(data.getElementsByTagName("value")[0]);
                  // console.log("id " + id);
       }
       catch(err){
      console.log('error on XML Parse::'+err);
        }
