	  ...
	  if (startDate == undefined){
		  ...
		  var pageName = '<s:property value="#session.sessionDTO.pageName"/>';
               ...
               $(document).ready(function() {
                  ...
                  $('#reportrangeResponsive').daterangepicker(
                     ...
                     function(start, end) {
						...
						if($("#compare_checbox").prop('checked') == true){
							...
							if ($('.ranges .active').text() == 'Custom Range') {
							 ...
							 $('#reportrangeResponsive div').html(user_startdate + ' - ' +user_enddate +'</br>');
							 $('#reportrangeResponsive p').html('Compared to:' + formated_todaytoyesterdaystart + ' - ' + formated_todaytoyesterdayend);
							...
							if($('.ranges .active').text() != 'Custom Range'){
								...
								$('#reportrangeResponsive div').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY') +'</br>');
								$('#reportrangeResponsive p').html('Compared to:' + formated_todaytoyesterdaystart + ' - ' + formated_todaytoyesterdayend);
		           ...
		           compareStartDate = compareStart.getFullYear()+"-"+compareStartMonth+"-"+compareStartDay 
		           	compareEndDate = compareEnd.getFullYear()+"-"+compareEndMonth+"-"+compareEndDay
		           	 ...
		           	 //endDate = startDate2.format('YYYY-MM-DD');
		           	  //var startDate1= 
		           		$('#reportrangeResponsive div').html(startDate1 + ' - ' + startDate2);
                  		$('#reportrangeResponsive p').html('Compared to:' + compareDate1 + ' - ' + compareDate2);
		           	  }
		           	else 
		           	  {
							...
							$('#reportrangeResponsive div').html(moment().subtract('days', 6).format('MMMM D, YYYY') + ' - ' + moment().format('MMMM D, YYYY'));
		                  	$('#reportrangeResponsive p').html('Compared to:' + moment().subtract('days', 13).format('MMMM D, YYYY') + ' - ' + moment().subtract('days', 7).format('MMMM D, YYYY'));
               ...
               <!-- <div class="well" style="float:right;margin-top: 10px;"> -->
 				<div class="well" style="float:right;margin-top: 10px;">
                <div id="reportrangeResponsive" class="pull-right" style="background: #fff; cursor: pointer; padding: 0px 3px; border: 1px solid #ccc">
